...
import { Knave2eActor, Knave2eChatMessage, Knave2eItem } from './documents/_module.mjs';
...
Hooks.on("updateActor", async (actor) => {
    ...
    await actor.updateEmbeddedDocuments("Item",
        actor.items.map(i => ({
        _id: i.id,
        "system.held": Math.max(i.system.quantity, 0),
        "system.dropped": i.system.quantity <= 0
        }))
    );
});
/* -------------------------------------------- */
