import { onAttack, onCast, onDamageFromSheet } from '../helpers/items.mjs';
...
export default class Knave2eActorSheet extends foundry.appv1.sheets.ActorSheet {
    ...
    activateListeners(html) {
        ...
        root.querySelectorAll('.item-edit').forEach(el => {
            el.addEventListener('click', ev => {
                const li = ev.currentTarget.closest('.knave-item');
        ...
        root.querySelectorAll('.item-create').forEach(el => {
    ...
    // Drag events for macros.
        if (this.actor.isOwner) {
            ...
            root.querySelectorAll('li.item').forEach(li => {
                ...
                li.addEventListener('dragstart', handler);
            });
        }
        root.addEventListener('click', ev => {
            ...
            if (el.classList.contains('rollable')) return this._onRollable(ev);
            
            if (el.classList.contains('item-name')) return this._onItemName(e);
            if (el.classList.contains('quantity')) return this._onQuantity(e);
            if (el.classList.contains('item-toggle')) return this._onItemToggle(e);
            
            if (el.classList.contains('attack')) return onAttack.call(this, e);
            if (el.classList.contains('damage')) return onDamageFromSheet.call(this, e);
            if (el.classList.contains('cast')) return onCast.call(this, e);
            
            if (el.classList.contains('check')) return this._onCheck(e);
            if (el.classList.contains('ap')) return this._onAP(e);
            if (el.classList.contains('rest')) return this._onRest(e);
            if (el.classList.contains('morale')) return this._onMorale(e);
            if (el.classList.contains('numberAppearing')) return this._onNumberAppearing(e);        });
    }
    ...
    const dataset = element.dataset;
        ...
        if (dataset.dataType) {
            if (dataset.dataType == 'item') {
                const itemId = element.closest('.knave-item')?.dataset.itemId;
