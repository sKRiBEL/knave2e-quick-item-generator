...
if (this.actor.type !== 'character') {
    ...
    const shift = event.shiftKey ?? event.originalEvent?.shiftKey ?? false;
    ...
    await r.evaluate();
    // --- ADDED FOR DICE SO NICE ---
    if (game.dice3d) {
        // This line makes the 3D dice roll and waits for them to land
        await game.dice3d.showForRoll(r, game.user, true, null, false);
    }
    // ------------------------------
...
export async function onDamageFromSheet(event) {
    ...
    _rollDamage(a, actor, item, event);
}

async function _rollDamage(a, actor, item, event) {
    const systemData = actor.system;
    const itemData = item.system;
        ...
        if (shift === false) {
            ...
            if (powerAttack) {
                ...
                if (game.settings.get('knave2e', 'enforceBreaks') && itemData.breakable) {
                    const newBQ = (itemData.brokenQuantity ?? 0) + 1;
                    const qty = itemData.quantity ?? 1;
                    //power attacks break item
                    await item.update({
                    'system.brokenQuantity': newBQ,
                    'system.broken': newBQ >= qty
                    });
                    if (itemData.broken) {
                        rollFlavor =
                            rollFlavor +
                            `. A ${game.i18n.localize('KNAVE2E.PowerAttack')} was attempted. <b>However, this ${item.name} is broken!</b>`;
                    }
                    else {
                        //TODO: localize this string
                        rollFlavor =
